...
interface CollectionCreateFormProps {
  ...
  onCreate: (type: string, values: ERC20) => void;
  onCancel: () => void;
}
...
const ERC20: React.FC = () => {
    ...
    const onCreate = (type: any, values: any) => {
      ...
      request.post(`${batchAirdropUrlPrefix}/erc20${urlObj[type]}`, {
        data: param,
      })
    ...
    const handleDelete = (key: React.Key) => {
      ...
      request.post(`${batchAirdropUrlPrefix}/erc20/delete`, {
        data: { id : key},
      })
      .then(function(response) {
        if (response.code === 0) {
    ...
    // 表格的列
    const columns: ProColumns<ERC20>[] = [
      // {
      //   title: 'ID',
      //   dataIndex: 'id',
      //   // copyable: true,
        ...
        <>
            <ProTable<ERC20>
              ...
              request={async (params = {}, sort, filter) => {
                  ...
                  await request<{
                      data: ERC20[];
                  }>(`${batchAirdropUrlPrefix}/erc20/list`, {
                    params,
                  }).then(r => ret = r);
