...
const Keyless: React.FC = () => {
  const actionRef = useRef<ActionType>();
  const [messageApi, contextHolder] = message.useMessage();
  const { initialState } = useModel('@@initialState');
  const [form] = Form.useForm();

  // const [currInitNetworkId, setCurrInitNetworkId] = useState<{[key: string]: any}>([]);
  // const [currInitNetworkId, setCurrInitNetworkId] = useState("");
  ...
  const handleKeystoreOk = () => {
    ...
    form
      ...
      .then((values) => {
        ...
        request.post(`${create2UrlPrefix}/keyless/init`, {
          data: { 
          ...
          .then(function(response) {
            ...
            if (response.code === 0) {
              ...
              // setCurrInitNetworkId(values.network_id)
  ...
  const doInit = (key: React.Key) => {
    console.log('do keyless init', key);
  ...
  // 表格的列
  const columns: ProColumns<KeylessItem>[] = [
    {
      title: '链',
      key: 'network_name',
    ...
    <>
      ...
      <ProTable<KeylessItem>
        ...
        request={async (params = {}, sort, filter) => {
          ...
          //   console.log(res.data[i].network_id , +currInitNetworkId, +res.data[i].network_id === +currInitNetworkId, res.data[i].keyless_address);
          //   if (+res.data[i].network_id === +currInitNetworkId && res.data[i].keyless_address) {
          //     clearInterval(handler)
          //     break
          //   }
          // }
